...
const About = () => {
  ...
  return (
    <section id="about" ref={sectionRef} className="py-[120px] relative overflow-hidden">
      <img ref={maskRef} src={maskImg} alt="" className="absolute left-[-80px] top-1/2 -translate-y-1/2 h-[80%] w-auto opacity-[0.08] pointer-events-none z-0" style={{ mixBlendMode: isDark ? 'screen' : 'multiply' }} />
      ...
      <div className="relative z-[1] max-w-[1100px] mx-auto px-10 grid grid-cols-2 gap-20 items-center">
        ...
        <div ref={photoRef} className="relative w-fit">
          ...
          <div className="flex gap-4 mt-5">
            {['GitHub', 'LinkedIn', 'Twitter'].map(s => (
              <a key={s} href="#" className="font-mono text-[12px] text-foreground/50 hover:text-gold transition-colors">{s}</a>
            ))}
          </div>
        </div>
        ...
        <div>
          ...
          <h2 className="font-playfair text-[42px] text-foreground mt-2 leading-[1.1]">I code with intent.</h2>
          <div className="mt-6 space-y-4 font-sans text-[16px] leading-[1.7] text-foreground/70">
          ...
          <div className="stats-row grid grid-cols-3 gap-6 mt-10 border-t border-border pt-8">
            {stats.map(s => (
              <div key={s.label}>
                <p className="font-playfair text-[44px] text-crimson">{s.num}</p>
                <p className="font-mono text-[11px] text-gold uppercase">{s.label}</p>
                <p className="font-sans text-[13px] text-foreground/40">{s.sub}</p>
              </div>
            ))}
          </div>
        </div>
